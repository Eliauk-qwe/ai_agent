...
DashScope 是阿里云的大模型服务平台（灵积），qwen-plus 就是跑在上面的。

**你的代码最终都是通过 HTTP 请求打到阿里云的服务器，让那边的模型来推理生成回答，再把结果返回给你。**

你的代码 ──HTTP──▶ 阿里云 DashScope 服务器 ──▶ qwen-plus 模型推理 ──▶ 返回结果
四种调用方式的区别只是**怎么发这个 HTTP 请求**：

- Hutool — 手动拼 HTTP
- DashScope SDK — 阿里官方封装的 HTTP 客户端
- LangChain4j — 第三方 AI 框架封装
- Spring AI — Spring 官方 AI 框架封装
**最终打到的都是同一个阿里云接口。**
